...
public class GetFireworkCommand {
                                ...
                                .then(Commands.argument("power", IntegerArgumentType.integer(1, 3))
                                        .then(Commands.argument("color", StringArgumentType.string())
                                                .executes(context -> {
                                                    ...
                                                    return giveFireworks(context.getSource(), count, explode, power, color);
                                                }))))));

        dispatcher.register(Commands.literal("getfw")
                .then(Commands.argument("count", IntegerArgumentType.integer(1, 64))
                        .then(Commands.argument("explode", BoolArgumentType.bool())
                                .then(Commands.argument("power", IntegerArgumentType.integer(1, 3))
                                        .then(Commands.argument("color", StringArgumentType.string())
    ...
    private static int giveFireworks(CommandSourceStack source, int count, boolean explode, int power, String color) {
        if (source.getEntity() instanceof ServerPlayer player) {
            ItemStack fireworkStack = createFireworkStack(count, explode, power, color);
    ...
    private static ItemStack createFireworkStack(int count, boolean explode, int power, String color) {
        ...
        fireworkTag.putByte("Flight", (byte) power);
